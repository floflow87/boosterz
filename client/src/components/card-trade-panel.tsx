...
import { X, DollarSign, Handshake } from "lucide-react";
...
export default function CardTradePanel({ card, isOpen, onClose }: CardTradePanelProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateTradeMutation.mutate({
      tradeDescription,
      tradePrice: tradeOnly ? "" : tradePrice,
      tradeOnly,
      isForTrade
    });
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="flex items-center space-x-2">
            <Checkbox
              id="isForTrade"
              checked={isForTrade}
              onCheckedChange={(checked) => setIsForTrade(checked as boolean)}
            />
            <Label htmlFor="isForTrade" className="text-white">
              Mettre cette carte en trade
            </Label>
          </div>
